...
# This script analyses different cell proliferation scenarios to find the time taken for dividing cells to repolarize
...
nx = 8          #Number of cells along x-direction
...
Lx = nx*cd + 40      #for left boundary signal configuration 
#
#POTTS PARAMETERS:
T=15            #Temperature 
nOrder_contact=4 #Distance of interaction 
nOrder_potts=3  #Distance of interaction 
Time=500000     #Total number of MCS
#
...
[[gr=[0.1,0.01,0.001]]]
growth_rate= [[gr]]                 #rate of growth of cells during proliferation
...
[[gr_criteria=[0,1,2]]]
growth_criteria = [[gr_criteria]] 
#0 - GROWTH ONE COLUMN FRONT
#growth_criteria = 1 #GROW ONE COLUMN INSIDE
#growth_criteria = 2 #GROW SINGLE CELL INSIDE

#axis_to_divide = 0 #0- MINOR AXIS DIVISION, 1-MAJOR AXIS DIVISION
[[axis=[0,1]]]
axis_to_divide = [[axis]] #MAJOR AXIS DIVISION
#
def configure_simulation():
    ...
    SteppableElmnt.ElementCC3D("PIFName",{},"Simulation/PiffFile_15000.piff")
...
CompuCellSetup.register_steppable(steppable=GraphGlobalPol(10,cd,MCS0,growth_criteria))                                         #Computes the global polarization (Phi) by averaging individual cell polarity vectors and saves the data into a .txt file
CompuCellSetup.register_steppable(steppable=Growth(1,cd,tV,LamV,pP,pD,pC,growth_rate,t_relax,growth_criteria))  #Implements cell growth for cell proliferation on front
CompuCellSetup.register_steppable(steppable=Mitosis(1,cd,tV,LamV,pP,pD,pC,t_relax,axis_to_divide))             #Implements cell division for cell proliferation on front
# CompuCellSetup.register_steppable(steppable=piff_generator(1000,Time,MCS0))                                    Generates and stores .piff files
