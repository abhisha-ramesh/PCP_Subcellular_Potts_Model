...
def local_pol(N_sim,n_cells):
    local_pol_data = np.zeros((N_sim, n_cells-1))
    cell_number_list = np.zeros(n_cells-1)
    n_files = 0
    str_n_cells = str(n_cells)
    for n in range(1, N_sim + 1):
        str_n = "%03d" % n
        filename = f'/share/belmonte/athayam/CC3DWorkSpace/PCP__N_{str_n_cells}__2025_Feb_15__22_14_16/0{str_n}/localpol.txt'
        ...
        if os.path.isfile(filename):
            ...
            n_files += 1
            if data.size > 0:
                for n_cell in range(1,n_cells):
                    local_pol_data[n-1,(n_cells - n_cell-1)] = data[-n_cell,1]
    ...
    np.savetxt("/share/belmonte/athayam/PCP_main_paper/system_size_data/localpol_periodic_N_"+str(n_cells)+".csv", np.c_[cell_number_list, L], delimiter=",", header="radius, localpol")
