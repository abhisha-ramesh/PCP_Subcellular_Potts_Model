...
global LamV, tV
global pP, pD, pC
global E_pd, E_dd, E_pp, E_ll, E_lp, E_ld, E_ml, J_pd, E_pd_mut, J_pd_mut, E_pmut

#PARAMETERS:
cd=12           #Typical cell diameter
#Lx=8*cd        #Size of lattice (Lx X Ly) for periodic boundary conditions
Lx=(12*cd)+cd   #Size of lattice (Lx X Ly) for left boundary signal
...
def configure_simulation():
    ...
    PluginElmnt_4.ElementCC3D("Energy",{"Type1":"mut","Type2":"mut"},E_pp)    #because mutant cell type is proximal(Fz mutant)
    PluginElmnt_4.ElementCC3D("Energy",{"Type1":"mut","Type2":"prox"},E_pmut)
    PluginElmnt_4.ElementCC3D("Energy",{"Type1":"mut","Type2":"dist"},E_pd_mut)
    ...
    PluginElmnt_5.ElementCC3D("Energy",{"Type1":"mut","Type2":"dist"},J_pd_mut)
...
CompuCellSetup.register_steppable(steppable=piff_generator(1000,Time,MCS0))
CompuCellSetup.run()
